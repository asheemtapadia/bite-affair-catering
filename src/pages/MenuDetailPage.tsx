    ...
    <div className="min-h-screen">
      ...
      <div className="bg-navy pt-28 pb-12">
        <div className="container mx-auto px-4">
          <Link to="/#menu" className="inline-flex items-center gap-1 font-body text-sm text-navy-foreground/60 hover:text-primary transition-colors mb-4">
          ...
          <p className="font-heading text-2xl font-bold text-primary mt-2">
            ₹{pkg.price}<span className="text-base font-body font-normal text-navy-foreground/60">/- per person</span>
          </p>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
              ...
              <ScrollReveal key={cat.name} delay={0}>
                <div>
                  ...
                  <ul className="space-y-2">
                    {cat.items.map((item) => (
                      <li key={item} className="font-body text-sm text-foreground/80 py-1.5 border-b border-border/40 last:border-0">
                        {item}
                      </li>
                    ))}
                  </ul>
          ...
          <ScrollReveal delay={0.1}>
            <div className="mt-16 bg-beige rounded-lg p-6 md:p-8">
              <h3 className="font-heading text-lg font-semibold text-navy mb-4">Additional Information</h3>
              <ul className="space-y-2">
                {othersInfo.map((info) => (
